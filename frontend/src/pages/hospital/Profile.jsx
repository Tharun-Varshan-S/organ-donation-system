        ...
        <div className="profile-page">
            <form onSubmit={handleSave} className="profile-form">
                <div className="profile-status-badges">
                    <div className={`status-badge emergency-badge ${profile.isEmergencyReady ? 'active' : 'inactive'}`}>
                        <span className="badge-icon">🚨</span>
                        <div className="badge-content">
                            <span className="badge-label">Emergency Ready</span>
                            <span className="badge-value">{profile.isEmergencyReady ? 'Yes' : 'No'}</span>
                        </div>
                    </div>
                    <div className={`status-badge approval-badge ${profile.status === 'approved' ? 'approved' : 'pending'}`}>
                        <span className="badge-icon">{profile.status === 'approved' ? '✓' : '⏳'}</span>
                        <div className="badge-content">
                            <span className="badge-label">Status</span>
                            <span className="badge-value">{profile.status.toUpperCase()}</span>
                        </div>
                    </div>
                </div>

                {
                    profile.adminRemarks && (
                        <div className="admin-remarks-section">
                            <div className="admin-remarks-header">
                                <span className="remarks-icon">📝</span>
                                <h3>Admin Remarks (Read-Only)</h3>
                            </div>
                            <div className="admin-remarks-content">
                                {profile.adminRemarks}
                            </div>
                        </div>
                    )
