...
import { MapPin, Check, Star, ShieldCheck, CheckCircle2, X, Siren, AlertCircle } from 'lucide-react';
    ...
    return (
        ...
        >
            ...
            <div className="admin-card-image-wrapper">
                ...
                {isEmergency && (
                    <div className="absolute top-4 right-4 bg-red-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl animate-pulse flex items-center gap-2 border border-red-400">
                        <Siren size={14} /> Emergency Active
                    </div>
                )}
            </div>
            ...
            <div className="admin-card-details">
                ...
                <div className="admin-card-header">
                    <div>
                        ...
                        {isEmergency && hospital.capacity?.availableBeds > 0 && (
                            <p className="text-[10px] font-bold text-red-500 uppercase mt-1">
                                {hospital.capacity.availableBeds} CRITICAL BEDS AVAILABLE
                            </p>
                        )}
                    </div>
                ...
                <div className="flex gap-2 my-2">
                    <div className="flex-1 bg-gray-50 p-2 rounded-xl text-center border border-gray-100 group-hover:bg-white transition-colors">
                    ...
                    <div className="flex-1 bg-gray-50 p-2 rounded-xl text-center border border-gray-100 group-hover:bg-white transition-colors">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Requests</p>
                ...
                <div className="feature-list">
                    {hospital.specializations?.slice(0, 2).map((spec, i) => (
                        <div key={i} className="feature-item">
                            <CheckCircle2 size={12} className="check-icon text-blue-500" />
                            {spec}
                        </div>
                    ))}
                    {isEmergency && (
                        <div className="feature-item text-red-600">
                            <AlertCircle size={12} />
                            Rapid Response Unit
                        </div>
                    )}
                ...
                {/* Footer / Actions */}
                <div className="admin-card-footer mt-auto pt-4 border-t border-gray-50">
                    {hospital.status === 'pending' && (onApprove || onReject) ? (
                        <div className="flex gap-2 w-full">
                            <button
                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${actionLoading === 'approve' ? 'bg-gray-100 text-gray-400' : 'bg-blue-600 text-white hover:bg-black hover:shadow-xl'}`}
                                onClick={(e) => handleAction(e, 'approve')}
                                disabled={!!actionLoading}
                            >
                                {actionLoading === 'approve' ? <span className="animate-spin text-lg">◌</span> : <><Check size={14} /> Validate</>}
                            </button>
                            <button
                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${actionLoading === 'reject' ? 'bg-gray-100 text-gray-400' : 'border-red-200 text-red-600 hover:bg-red-50'}`}
                                onClick={(e) => handleAction(e, 'reject')}
                                disabled={!!actionLoading}
                            >
                                {actionLoading === 'reject' ? <span className="animate-spin text-lg">◌</span> : <><X size={14} /> Defer</>}
                            </button>
                        </div>
                    ) : (
                        <>
                            <div className="status-badge-container">
                                <span className={`status-badge ${hospital.status} text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-lg`}>{hospital.status}</span>
                            </div>
                            <div className="text-right">
                                <div className="footer-price text-[#1e293b] font-black">{hospital.rating || '4.8'}/5.0</div>
                                <div className="footer-label text-[10px] font-bold text-gray-400 uppercase">System Trust Score</div>
                            </div>
                        </>
                    )}
                </div>
